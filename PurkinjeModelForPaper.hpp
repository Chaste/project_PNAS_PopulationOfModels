...
#ifndef PURKINJEMODELFORPAPER_HPP_
#define PURKINJEMODELFORPAPER_HPP_
...
class PurkinjeModelForPaper : public AbstractModelPopCvodeSystem
{

public:
	PurkinjeModelForPaper() :
		AbstractModelPopCvodeSystem(29)
	{
		mpSystemInfo = OdeSystemInformation<PurkinjeModelForPaper>::Instance();
		Init();
	}

	//Constructor with initial conditions file specified
	PurkinjeModelForPaper(std::string initialConditionsFileName) :
		AbstractModelPopCvodeSystem(29)
	{
		mpSystemInfo = OdeSystemInformation<PurkinjeModelForPaper>::Instance();
